...
function SongSearch({ language }) {
  ...
  useEffect(() => {
    ...
    const fetchSongs = async () => {
      ...
      try {
        let url;
        if (query.trim() === '') {
          url = `${import.meta.env.VITE_API_URL}/songs?language=${encodeURIComponent(language)}`;
  ...
  const fetchMoreSongs = async () => {
    ...
    try {
      let url;
    if (query.trim() === '') {
      url = `${import.meta.env.VITE_API_URL}/songs?language=${encodeURIComponent(language)}&cursor=${encodeURIComponent(nextCursor)}`;
    } else {
      url = `${import.meta.env.VITE_API_URL}/songs/search?language=${encodeURIComponent(language)}&q=${encodeURIComponent(query)}&cursor=${encodeURIComponent(nextCursor)}`;
    }
      const res = await fetch(url);
      const data = await res.json();
